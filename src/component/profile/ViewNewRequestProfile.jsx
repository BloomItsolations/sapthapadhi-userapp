  ...
  const acceptUser = async (fromUserId) => {
    try {
      ...
      const response = await axios.post(`${process.env.REACT_APP_BaseURL}/app/update-request-status`,{
        ...
        console.log("response from viewrequest accepted",response)
    ...
    <>
      ...
      <div className=' w-full h-[220px] md:h-[420px] lg:h-[595px] flex flex-col justify-end items-center' style={{
        ...
        <div className='flex justify-center gap-5 mt-2 md:mt-3 lg:mt-4 mb-4 md:mb-10 lg:mb-14'>
              ...
              <button onClick={()=>acceptUser(userDetails.user.id)} className='w-[120px] md:w-[200px] lg:w-[281px] h-[35px] md:h-[40px] lg:h-[44px] rounded-[10px] bg-[#183D9E] text-white text-[13px] md:text-[20px] lg:text-[30px] font-medium font-[Inter] flex justify-center items-center '>Accept Request</button>    
        </div>
      </div>
...
export default ViewNewRequestProfile;